          ...
          {isAuthenticated === null ? (
            ...
            <>
              ...
              <div className="flex flex-col w-full gap-2">
                <Button asChild>
                  <Link href={`/login?redirectTo=${encodeURIComponent(window.location.href)}`}>Log In</Link>
                </Button>
                <Button variant="outline" asChild>
                  <Link href={`/register?redirectTo=${encodeURIComponent(window.location.href)}`}>Sign Up</Link>
                </Button>
              </div>
            </>
          ) : status === "loading" ? (

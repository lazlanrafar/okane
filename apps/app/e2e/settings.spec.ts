...
test.describe('System: Settings — Profile', () => {
  ...
  test('should render the profile settings page', async ({ page, dictionary }) => {
    ...
    await expect(page.getByRole('button', { name: dictionary.settings.profile.update_profile })).toBeVisible({ timeout: 15000 });
...
test.describe('System: Settings — Appearance', () => {
  ...
  test('should switch theme to Dark', async ({ page, dictionary }) => {
    const darkButton = page.getByRole('radio', { name: dictionary.settings.appearance.theme.dark }).or(page.getByText(dictionary.settings.appearance.theme.dark)).first();
...
test.describe('System: Settings — Billing', () => {
  ...
  test('should display Available Plans or Add-ons section', async ({ page, dictionary }) => {
    const section = page.getByText(dictionary.settings.billing.available_plans).or(page.getByText(dictionary.settings.billing.addons)).first();
...
test.describe('System: Settings — Transaction', () => {
  test.beforeEach(async ({ page, dictionary }) => {
    ...
    await expect(page.getByRole('heading', { name: dictionary.settings.transaction.title }).first()).toBeVisible({ timeout: 20000 });
  });

  test('should render the transaction settings page', async ({ page, dictionary }) => {
    await expect(page.getByRole('heading', { name: dictionary.settings.transaction.title }).first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should display Date & Time section', async ({ page, dictionary }) => {
    ...
    await expect(page.getByText(dictionary.settings.transaction.date_time.title).first()).toBeVisible({ timeout: 10000 });
  });

  test('should display Input & Interaction section', async ({ page, dictionary }) => {
    await expect(page.getByText(dictionary.settings.transaction.input_interaction.title).first()).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe('System: Settings — Wallets & Banks', () => {
  ...
  test('should render the wallets settings page', async ({ page, dictionary }) => {
    // Wait for hydration signal
    await expect(page.getByRole('heading', { name: dictionary.settings.wallets.title }).or(page.getByText(dictionary.settings.wallets.title))).toBeVisible({ timeout: 15000 });
  });
});

...
test.describe('Workspace: Contacts', () => {
  ...
  test('should open the New Contact form sheet', async ({ page, dictionary }) => {
    ...
    await page.getByRole('button', { name: dictionary.contacts.add_button }).filter({ visible: true }).first().click({ force: true });
...
test.describe('Finance: Debts & Receivables', () => {
  ...
  test('should show validation error when submitting empty debt form', async ({ page, dictionary }) => {
    ...
    await expect(
      page.getByText(dictionary.debts.form.contact.error_required).or(page.getByText(dictionary.debts.form.amount.error_required)).first()

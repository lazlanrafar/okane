...
test.describe('Finance: Income Categories', () => {
  ...
  test.beforeEach(async ({ page, dictionary }) => {
    ...
    await expect(page.getByRole('button', { name: dictionary.settings.category.income.add_button })).toBeVisible({ timeout: 15000 });
  });

  test('should render the income categories page', async ({ page, dictionary }) => {
    // CategoryForm uses h2 for the title
    await expect(page.getByRole('heading', { name: dictionary.settings.category.income.title, level: 2 })).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should open the Add Category form', async ({ page, dictionary }) => {
    ...
    await expect(page.getByPlaceholder(dictionary.settings.category.income.form.name.placeholder)).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should show validation error for empty category name', async ({ page, dictionary }) => {
    ...
    await expect(page.getByText(dictionary.settings.category.income.form.name.error_required)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should create a new income category', async ({ page, dictionary }) => {
    ...
    await expect(page.getByText(dictionary.settings.category.income.form.create_success)).toBeVisible({ timeout: 15000 });
  });
});
...
test.describe('Finance: Expense Categories', () => {
  ...
  test.beforeEach(async ({ page, dictionary }) => {
    ...
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByRole('button', { name: dictionary.settings.category.expense.add_button })).toBeVisible({ timeout: 15000 });
  });

  test('should render the expense categories page', async ({ page, dictionary }) => {
    // CategoryForm uses h2
    await expect(page.getByRole('heading', { name: dictionary.settings.category.expense.title, level: 2 })).toBeVisible({ timeout: 10000 });
  });

  test('should open the Add Category form', async ({ page, dictionary }) => {
    await page.getByRole('button', { name: dictionary.settings.category.expense.add_button }).click();
    await expect(page.getByPlaceholder(dictionary.settings.category.expense.form.name.placeholder)).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should show validation error for empty category name', async ({ page, dictionary }) => {
    ...
    await expect(page.getByText(dictionary.settings.category.expense.form.name.error_required)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should create a new expense category', async ({ page, dictionary }) => {
    ...
    await expect(page.getByText(dictionary.settings.category.expense.form.create_success)).toBeVisible({ timeout: 15000 });
  });
});

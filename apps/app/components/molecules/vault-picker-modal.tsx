      ...
      <DialogContent className="flex max-h-[70vh] flex-col sm:max-w-[900px]">
        ...
        <div className="flex shrink-0 flex-col gap-4 pt-6">
          <div className="flex items-center justify-between gap-4">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="h-10 w-full rounded-lg bg-transparent pl-9"
                value={search}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => setSortBy("newest")}>
                    Newest first {sortBy === "newest" && <Check className="ml-auto h-4 w-4" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortBy("name_asc")}>
                    Name (A-Z) {sortBy === "name_asc" && <Check className="ml-auto h-4 w-4" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortBy("name_desc")}>
                    Name (Z-A) {sortBy === "name_desc" && <Check className="ml-auto h-4 w-4" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortBy("size_desc")}>
                    Size (Largest) {sortBy === "size_desc" && <Check className="ml-auto h-4 w-4" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortBy("size_asc")}>
                    Size (Smallest) {sortBy === "size_asc" && <Check className="ml-auto h-4 w-4" />}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-40">
                  <DropdownMenuItem onClick={() => setView("list")} className="justify-between">
                    <span className="flex items-center">
                      <ListIcon className="mr-2 h-4 w-4 text-muted-foreground" /> List view
                    </span>
                    {view === "list" && <Check className="h-4 w-4" />}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setView("grid")} className="justify-between">
                    <span className="flex items-center">
                      <Grid className="mr-2 h-4 w-4 text-muted-foreground" /> Grid view
                    </span>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => setFilterType("all")}>
                  All types {filterType === "all" && <Check className="ml-auto h-4 w-4" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterType("image")}>
                  Images {filterType === "image" && <Check className="ml-auto h-4 w-4" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterType("document")}>
                  Documents {filterType === "document" && <Check className="ml-auto h-4 w-4" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterType("video")}>
                  Videos {filterType === "video" && <Check className="ml-auto h-4 w-4" />}
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => setFilterSize("all")}>
                  Any size {filterSize === "all" && <Check className="ml-auto h-4 w-4" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterSize("small")}>
                  Under 1MB {filterSize === "small" && <Check className="ml-auto h-4 w-4" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterSize("medium")}>
                  1MB - 5MB {filterSize === "medium" && <Check className="ml-auto h-4 w-4" />}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setFilterSize("large")}>
                  Over 5MB {filterSize === "large" && <Check className="ml-auto h-4 w-4" />}
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
          ...
          >
            ...
            <p className="text-muted-foreground text-sm">Drag and drop images, videos, 3D models, and files</p>
          </div>
        </div>
                      ...
                      >
                        <div className="flex shrink-0 items-center">
                          <button
                            ...
                            className={cn(
                              "flex h-5 w-5 cursor-pointer items-center justify-center rounded border shadow-sm transition-colors",
                              selected
                        ...
                        >
                          ...
                          <div className="absolute top-2 left-2 z-10 transition-opacity">
                            <button
                              ...
                              className={cn(
                                "flex h-5 w-5 items-center justify-center rounded border shadow-sm transition-all",
                                selected
